...
void IGameController::Clear()
{
    ...
    m_CurrentWave = 1;
    m_QueuedMonsters = 1;
    m_PreparTimer = 1; // 1 Because if not, the wave changes to 2 immediatly when a player looses (instead of staying at 1)
}
...
{
	// default health
	//pChr->IncreaseHealth(!pChr->GetPlayer()->m_DeathHealth ? pChr->GetPlayer()->TotalHP() : pChr->GetPlayer()->m_DeathHealth); // its annoying with deathhealth
	...
	pChr->m_MineNum = g_Config.m_SvMaxMines;
	pChr->m_WallNum = g_Config.m_SvMaxWalls;

	if(pChr->GetPlayer()->m_Upgrades.m_Protector)
        new CProtector(&GameServer()->m_World, pChr->GetPlayer()->GetCID());
}
...
void IGameController::Tick()
{
	...
	if(!m_MonsterSpawnNum && Server()->Tick() % (Server()->TickSpeed() * 5) == 0)
        GameServer()->SendChatTarget(-1, "Invalid map. Please use a map where there is at least 1 red player spawn entity.");
...
void IGameController::DoWincheck()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i ++)
    {
        ...
        if(GameServer()->m_apPlayers[i]->m_IsDeadByMonster)
            continue;
...
void IGameController::Update()
{
    ...
    if(m_PreparTimer)
    {
        if(m_PreparTimer % Server()->TickSpeed() == 0)
        {
            ...
            char aBuf[256];
                str_format(aBuf, sizeof(aBuf), "Next wave : %d\n%d second%s left\n\nLook at \"/shop\" or at votes\nif you want to buy something",
                           m_CurrentWave, m_PreparTimer/Server()->TickSpeed(), m_PreparTimer/Server()->TickSpeed() > 1 ? "s" : "");
    ...
    if((m_QueuedMonsters <= 0 && !AliveMonsters))
        ChangeWave(m_CurrentWave + 1);
    else
    {
        for(int i = 0; i < MAX_MONSTERS; i ++)
        {
            if(!GameServer()->GetValidMonster(i) && m_QueuedMonsters)
            {
                NewMonster(i);
                m_AliveMonsters ++;
            }
...
{
    int Type = 0, Health = 0, Armor = 0, Difficulty = 1;
    if(m_CurrentWave <= 30)
    {
        Health = -1;
        if(m_CurrentWave <= 5)
            ...
            Type = TYPE_GUN;
        else if(m_CurrentWave <= 15)
        {
            Type = TYPE_SHOTGUN;
            Armor = 5;
        }
        else if(m_CurrentWave <= 20)
        {
            Type = TYPE_GRENADE;
            Armor = 5;
        }
        else if(m_CurrentWave <= 25)
        {
            Type = TYPE_LASER;
            Armor = 7;
        }
    ...
    else if(m_CurrentWave <= 55)
    {
        Difficulty = 2;
        Armor = 10;
        if(m_CurrentWave <= 35)
            Type = TYPE_GUN;
        else if(m_CurrentWave <= 40)
            Type = TYPE_SHOTGUN;
        else if(m_CurrentWave <= 45)
            Type = TYPE_GRENADE;
        else if(m_CurrentWave <= 50)
            Type = TYPE_LASER;
        else
            Type = TYPE_NINJA;
    }
    else if(m_CurrentWave <= 80)
    {
        Difficulty = 3;
        Armor = 10;
        if(m_CurrentWave <= 60)
            Type = TYPE_GUN;
        else if(m_CurrentWave <= 65)
            Type = TYPE_SHOTGUN;
        else if(m_CurrentWave <= 70)
            Type = TYPE_GRENADE;
        else if(m_CurrentWave <= 75)
            Type = TYPE_LASER;
        else
            Type = TYPE_NINJA;
    }
    else if(m_CurrentWave <= 105)
    {
        Difficulty = 4;
        Armor = 10;
        if(m_CurrentWave <= 85)
            Type = TYPE_GUN;
        else if(m_CurrentWave <= 90)
            Type = TYPE_SHOTGUN;
        else if(m_CurrentWave <= 95)
            Type = TYPE_GRENADE;
        else if(m_CurrentWave <= 100)
            Type = TYPE_LASER;
        else
            Type = TYPE_NINJA;
    }
    ...
    {
        Difficulty = (m_CurrentWave - 5) / 25 + 1;
        Armor = 10;
        Type = rand()%(NUM_WEAPONS-2) + 2;
    }

    Health += Difficulty * 3 - 2;
    ...
    if(MonsterID >= 0 && MonsterID < MAX_MONSTERS)
    {
        ...
        {
            m_QueuedMonsters --;
            GameServer()->m_apMonsters[MonsterID] = new CMonster(&GameServer()->m_World, Type, MonsterID, Health, Armor, Difficulty);
        }
    }
}

int IGameController::GetPlayerExtraHP() const
{
    if(m_CurrentWave >= 6)
        return m_CurrentWave / 5;

    return 0;
}

void IGameController::ChangeWave(int number)
{
    for(int i = 0; i < MAX_CLIENTS; i ++)
    {
        if(!GameServer()->m_apPlayers[i] || !GameServer()->m_apPlayers[i]->m_IsDeadByMonster)
            continue;
        ...
        if(GameServer()->m_apPlayers[i]->m_IsDeadByMonster)
        {
            GameServer()->m_apPlayers[i]->m_IsDeadByMonster = false;
            GameServer()->m_apPlayers[i]->SetTeam(TEAM_RED, false);
        }
    }

    m_CurrentWave = number;
    ...
    if(m_CurrentWave <= 30)
    {
        if((m_CurrentWave-1) % 5 == 0)
            m_QueuedMonsters = 1;
        else if((m_CurrentWave-2) % 5 == 0)
            m_QueuedMonsters = 3;
        else if((m_CurrentWave-3) % 5 == 0)
            m_QueuedMonsters = 7;
        else if((m_CurrentWave-4) % 5 == 0)
            m_QueuedMonsters = 12;
        else if(m_CurrentWave % 5 == 0)
            m_QueuedMonsters = 20;
    }
    else
        m_QueuedMonsters = (m_CurrentWave - 30) * 3 + 20;
    g_Config.m_GodMode = false;
}
